...
def main(args):
    ...
    for i, e in enumerate(energies):
        probabilities[i] = dos.get_weight(e)
    probabilities = 1.0 / probabilities
    probabilities = probabilities / np.sum(probabilities)

    posinp = Posinp.from_file("ortho_4_2.xyz")
    move_radius = 0.08

    displacements = np.zeros((args.n_structs, len(energies)))
    for i in range(args.n_structs):
        new_posinp = deepcopy(posinp)
        idx = np.random.choice(len(probabilities), 2, p=probabilities, replace=False)
        prop = np.random.rand()
        disp = prop * basis[:, idx[0]] + (1 - prop) * basis[:, idx[1]]
        #for j in range(int(len(disp) / 3)):
        #    disp[3*j: 3*j + 2] = (
        #        np.random.rand()
        #        * move_radius
        #        * disp[3*j: 3*j + 2]
        #        / np.max(disp[3*j: 3*j + 2])
        #    )
        #displacements[i] = disp

    with h5py.File("test.h5", "w") as f:
        f.create_dataset("displacements", data=displacements)

        # ajouter une normalisation des déplacements
        # autre méthode en choisissant l'énergie plutôt que les modes
...
#        posinp = generate_random_structure(positions[j], dos)
...
def generate_random_structure(initpos, dos):
    radius = 0.1
...
def create_parser():
    ...
    parser.add_argument(
        "dos", type=str, help="Path to the h5 file containing the phonon dos."
    )
    parser.add_argument(
        "basis",
        type=str,
        help="Path to the h5 file containing the normal modes or orthonormal basis obtained with the initial model.",
    )
    parser.add_argument(
